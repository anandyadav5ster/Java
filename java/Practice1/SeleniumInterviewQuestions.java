...
{
	TakeScreenshot srcshot = ((TakeScreenshot)WebDriver);
	File src = srcshot.getScreenshotAs(Output.File);
	FileUtils.copyFile(src,new File(pathname);
	
}

static void getWindowsHandlesApplication()
    {
       ...
       while(i.hasNext())
        {
            String cw = i.next();
            if(!cw.equalIgnoreCase(parentwindow)){
 ...
 static void HandleBrokenLinks()
    ...
    static void verifyUrl(String url)
    {
        ...
        {
             System.out.println(httpconnection.getResponseMessage()+"  is a broken link");
        }
    }
    catch(Exception e){
...
===========================================================
difference between selenim 3 and selenium 4 ?
1. More stability in cross browser testing.
2. Relative locators added (above, below, to the left, to the right)
3. New feature of javascript support added like executeAsyncScript.This will allow to work with javascript with promises and async/wait.
4. Improved event listners.
5. improve Selenium Grid4 scalability and stability.
6. Devtool for enhance debugging and analysis.
7. Automatic browser update.
